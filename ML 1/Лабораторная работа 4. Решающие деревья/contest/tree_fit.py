...
class MyDecisionTreeNode:
    ...
    def fit(self, X: np.ndarray, y: np.ndarray):
        ...
        """
        if (
            self._depth >= self._meta.max_depth or len(y)<self._meta.min_samples_split or self._impurity == 0 or len(y)<=1 or gini(y) == 0
        ):
            self._node_type = NodeType.TERMINAL
            self._predicted_class = mode(y)[0]
            self._class_proba = np.unique(y, return_counts=True)[1]/len(y)
            return self
