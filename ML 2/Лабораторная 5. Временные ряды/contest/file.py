...
def extract_advanced_features(
    ...
    for date in dates:
        date_features.extend([
            ...
            date.weekday(),  # День недели
            date.quarter,  # Квартал
            date.dayofyear,  # День года
            date.dayofyear / 366,  # Нормализованный день года
            # np.sin(2 * np.pi * date.dayofyear / 366),  # Синусоидальное представление дня года
            # np.cos(2 * np.pi * date.dayofyear / 366),  # Косинусоидальное представление дня года
            # date.is_month_end,  # Конец месяца (0/1)
            # date.is_month_start,  # Начало месяца (0/1)
            # date.is_quarter_end,  # Конец квартала (0/1)
            # date.is_quarter_start,  # Начало квартала (0/1)
    ...
    date_features.extend([
            ...
            next_date.weekday(),  # День недели
            next_date.quarter,  # Квартал
            next_date.dayofyear,  # День года
            next_date.dayofyear / 366,  # Нормализованный день года
            # np.sin(2 * np.pi * next_date.dayofyear / 366),  # Синусоидальное представление дня года
            # np.cos(2 * np.pi * next_date.dayofyear / 366),  # Косинусоидальное представление дня года
            # next_date.is_month_end,  # Конец месяца (0/1)
            # next_date.is_month_start,  # Начало месяца (0/1)
            # next_date.is_quarter_end,  # Конец квартала (0/1)
            # next_date.is_quarter_start,  # Начало квартала (0/1)
            # Признак выходного дня
...
def build_datasets(
    ...
    for i in range(model_count):
        ...
        for j in range(window_size + i, len(timeseries)):
            ...
            features = extract_features(timeseries[:j], i, window_size)
            X.append(features)
...
def predict(
    ...
    for i, model in enumerate(models):
        ...
        features = extract_features(current_ts, i)
...
def train_models(
    ...
    """
    window_size = 28*3  # Увеличенный размер окна для улучшения качества прогноза

    # Используем расширенные признаки для лучшего прогноза
    datasets = build_datasets(train_timeseries, extract_advanced_features, window_size, model_count)

    models = []

    for i, (X, y) in enumerate(datasets):
        # Для краткосрочных прогнозов используем градиентный бустинг
        # if i < 7:
        ...
        #         max_depth=4
        #     )
        # # Для долгосрочных прогнозов используем Ridge регрессию,
        # # так как она менее склонна к переобучению
        # else:
...
def score_models(
    ...
    for i in range(len(test_ts) - predict_len + 1):
        predictions.extend(list(predict(train_ts, models,extract_features)))
